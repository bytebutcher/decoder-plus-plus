...
class Plugin(ScriptPlugin):
    ...
    def __init__(self, context):
        ...
        self.config().add(PluginConfig.Option(
            Plugin.Option.Search_Term, "", ""))
        self.config().add(PluginConfig.Option(
            Plugin.Option.Replace_Term, "", ""))
        self.config().add(PluginConfig.Option(
            Plugin.Option.Should_Match_Case, True, "", False))
        self.config().add(PluginConfig.Option(
            Plugin.Option.Is_Regex, False, "", False))
        self._dialog = None
    ...
    def _get_search_term(self):
        return self.config().get(Plugin.Option.Search_Term).value

    def _get_replace_term(self):
        return self.config().get(Plugin.Option.Replace_Term).value

    def _should_match_case(self):
        return self.config().get(Plugin.Option.Should_Match_Case).value

    def _is_regex(self):
        return self.config().get(Plugin.Option.Is_Regex).value
...
class SearchAndReplaceDialog(QDialog):
    ...
    def _init_form_frame(self):
        ...
        self._search_term_edit = QLineEdit(self._config.get(Plugin.Option.Search_Term).value)
        ...
        self._replace_term_edit = QLineEdit(self._config.get(Plugin.Option.Replace_Term).value)
    ...
    def _on_config_change(self, event):
        self._config.update({
            Plugin.Option.Search_Term: self._get_search_term(),
            Plugin.Option.Replace_Term: self._get_replace_term(),
